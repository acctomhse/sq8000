...
double dtime(void)
{
	...
	rtc_get_time(&tm);		/* get current time from RTC */
//	printf("\n%01d%01d:%01d%01d:%01d%01d.%02d",
//		tm.ten_hr, tm.hr, tm.ten_min, tm.min, tm.ten_sec, tm.sec, tm.sos);
	q = (double)(tm.ten_min * 10 + tm.min) * 60.0 + (double)(tm.ten_sec * 10 + tm.sec) + ((double)tm.sos / 16.0);

	return q;
}
#endif /* CPU_ARCH_LEXRA || CPU_ARCH_MIPS */
