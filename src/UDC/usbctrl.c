...
//#define CONFIG_UDC_DEBUG
	...
	#define UDC_DBG(fmt, args...) printf("UDC: " fmt, ## args)
#else
...
//	if (bIsConnected)
//		UDC_DBG(" -> USB VBus connect\n");
//	else
//		UDC_DBG(" -> USB VBus disconnect");

	// enable interrupt
	request_irq(UDC_INT, UDCIintHandler, NULL);
...
//printf("BKI %x \n",virt_to_phy((u32_t)dma_src_addr));
...
//	DEBUG_OUT('s');
	...
	{
		if ( IS_HIGH_SPEED() ) {
			;//UDC_DBG(" USB connect with HIGH Speed!\n");
			bIsFullSpeed = 0;
			...
			;//UDC_DBG(" USB connect with FULL Speed!\n");
			bIsFullSpeed = 1;
		...
		for (ch = 0; ch < NUM_OF_USB_CHANNEL; ch++) {
			...
			//printf("BKO : %x \n",virt_to_phy( (u32_t)ChannelSet[ch].pBulkOutBuffer));
...
#error "****** END_POINTER_NUM is not well defined!"
#endif
		if ( bIsFullSpeed ) {
			;//UDC_DBG("Full speed config descriptor!\n");
			memcpy( CtrlInBuffer, fuspd_cfg_ptr, total_len);
		} else {
			;//UDC_DBG("High speed config descriptor!\n");
	...
	case INTF_DSCR_TYPE:
		total_len = 9;
		if( bIsFullSpeed ) {
			;//UDC_DBG("Full speed interface descriptor!\n");
			memcpy( CtrlInBuffer, fuspd_cfg_ptr + 9, total_len );
		} else {
			;//UDC_DBG("High speed interface descriptor!\n");
			memcpy( CtrlInBuffer, hispd_cfg_ptr + 9, total_len );
		}
		break;

	case EDP_DSCR_TYPE:
		;//UDC_DBG("EndPoiint descriptor!\n");
		total_len = 7;
...
#error "****** END_POINTER_NUM is not well defined!"
	...
	default:
		;//UDC_DBG("Get decsriptor : request error!\n");
		bRequestError = YES;
		break;
	}
...
void process_in0()
{
	...
	if ( uControlStage == DATA_STAGE ) {
        ...
        //;//UDC_DBG("tx0 handler uControlDataSize = %x\n",uControlDataSize); 
...
void process_bulk_out(int ch)
{
...
#if 1
	while ( (rxvalue & RxVOID  ) != 0 );
	while ( (rxvalue & RxERR   ) != 0 );
	while ( (rxvalue & RxOVF   ) != 0 );
#endif
	...
	if ( (rxvalue & RxACK   ) != 0 ) {
		...
		//printf("BKO : %x \n",virt_to_phy((u32_t)(ChannelSet[ch].pBulkOutBuffer + ChannelSet[ch].uCurrentBulkOutBlockNum * uBulkBlockSize)));
		writew( ENP_DMA_START, UDC_DMACTRLO(ChannelSet[ch].uBulk_OUT) );
...
#if 1
	while ( (TX_Status & (TxVOID)  ) != 0 );
	while ( (TX_Status & (TxURF)   ) != 0 );
#endif
...
//	UDC_DBG("int2flgReg = 0x%08x ", int2flgReg);
	...
	if ( int2flgReg & OUT0_INTR ) {
		process_out0();
		UDC_DBG("process_out0\n");
	}

	// Bulk_OUT Interrupt
	if ( int2flgReg & ENP1_INTR ) {
		process_bulk_out(0);
		UDC_DBG("process_bulk_out 1\n");
	}
//#if END_POINTER_NUM == 16
	if ( int2flgReg & ENP4_INTR ) {
		process_bulk_out(1);
		UDC_DBG("process_bulk_out 4\n");
	}
	if ( int2flgReg & ENP7_INTR ) {
		process_bulk_out(2);
		UDC_DBG("process_bulk_out 7\n");
	}
	if ( int2flgReg & ENP10_INTR ) {
		process_bulk_out(3);
		UDC_DBG("process_bulk_out 10\n");
	}
	if ( int2flgReg & ENP13_INTR ) {
		process_bulk_out(4);
		UDC_DBG("process_bulk_out 13");
	}
//#endif	/* END_POINTER_NUM */
	...
	if ( int2flgReg & ENP2_INTR ) {
		process_bulk_in(0);
		UDC_DBG("process_bulk_in 2\n");
	}
//#if END_POINTER_NUM == 16
	if ( int2flgReg & ENP5_INTR ) {
		process_bulk_in(1);
		UDC_DBG("process_bulk_in 5\n");
	}
	if ( int2flgReg & ENP8_INTR ) {
		process_bulk_in(2);
		UDC_DBG("process_bulk_in 8\n");
	}
	if ( int2flgReg & ENP11_INTR ) {
		process_bulk_in(3);
		UDC_DBG("process_bulk_in 11\n");
	}
	if ( int2flgReg & ENP14_INTR ) {
		process_bulk_in(4);
		UDC_DBG("process_bulk_in 14\n");
	}
//#endif	/* END_POINTER_NUM */
	...
	if ( int2flgReg & ENP3_INTR ) {
		process_intr_req(0);
		UDC_DBG("process_intr_req 3\n");
	}
//#if END_POINTER_NUM == 16
	if ( int2flgReg & ENP6_INTR ) {
		process_intr_req(1);
		UDC_DBG("process_intr_req 6\n");
	}
	if ( int2flgReg & ENP9_INTR ) {
		process_intr_req(2);
		UDC_DBG("process_intr_req 9\n");
	}
	if ( int2flgReg & ENP12_INTR ) {
		process_intr_req(3);
		UDC_DBG("process_intr_req 12\n");
	}
	if ( int2flgReg & ENP15_INTR ) {
		process_intr_req(4);
		UDC_DBG("process_intr_req 15\n");
	}
//#endif	/* END_POINTER_NUM */
}
